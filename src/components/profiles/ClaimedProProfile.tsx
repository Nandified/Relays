        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-400">({pro.reviewCount} reviews)</span>
            </div>
          </div>
        </div>
          ...
          <div className="mt-6">
            <div className="aspect-video rounded-2xl bg-[var(--bg-elevated)] border border-[var(--border)] flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">Intro video coming soon</p>
              </div>
            </div>
          </div>
        )}
        ...
        {pro.socialLinks && Object.values(pro.socialLinks).some(Boolean) && (
          <div className="mt-5 flex items-center gap-3">
            {pro.socialLinks.instagram && (
              <a href={`https://instagram.com/${pro.socialLinks.instagram}`} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-400 hover:text-pink-400 transition-colors" title="Instagram">
            ...
            {pro.socialLinks.linkedin && (
              <a href={`https://linkedin.com/in/${pro.socialLinks.linkedin}`} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-400 hover:text-blue-500 dark:text-blue-400 transition-colors" title="LinkedIn">
                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
            )}
            {pro.socialLinks.facebook && (
              <a href={`https://facebook.com/${pro.socialLinks.facebook}`} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-400 hover:text-blue-500 transition-colors" title="Facebook">
                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
            )}
            {pro.socialLinks.tiktok && (
              <a href={`https://tiktok.com/@${pro.socialLinks.tiktok}`} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors" title="TikTok">
            ...
            {pro.socialLinks.youtube && (
              <a href={`https://youtube.com/@${pro.socialLinks.youtube}`} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-400 hover:text-red-500 transition-colors" title="YouTube">
          ...
          <div className="flex flex-wrap gap-2">
            {pro.serviceAreas.map((area, i) => (
              <span key={area}>
                ...
                {i < pro.serviceAreas.length - 1 && <span className="text-slate-500 dark:text-slate-400 ml-2">•</span>}
              </span>
            ))}
          </div>
        </div>
