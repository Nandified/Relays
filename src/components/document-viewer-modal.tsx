      ...
      <div className="relative mx-4 w-full max-w-lg animate-in">
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] shadow-[var(--shadow-modal)] backdrop-blur-xl overflow-hidden">
          ...
          <div className="flex items-start justify-between p-6 pb-0">
            ...
            <button
              onClick={onClose}
              className="flex-shrink-0 rounded-full p-1.5 text-slate-500 hover:bg-black/5 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-slate-300 transition-colors ml-3"
            >
          ...
          <div className="p-6">
            ...
            {!hasFile && (
              <div className="mb-5 rounded-2xl border border-dashed border-[var(--border)] bg-black/[0.02] dark:bg-white/[0.02] p-8 text-center">
                <div className="flex flex-col items-center gap-2 text-slate-500">
                  <svg width="32" height="32" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="text-slate-500 dark:text-slate-600">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[var(--border)]">
                {doc.uploadedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Uploaded by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.uploadedBy}</span>
                  </div>
                )}
                {doc.uploadedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Uploaded</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{formatDate(doc.uploadedAt)}</span>
                  </div>
                )}
                {doc.requestedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Requested by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.requestedBy}</span>
                  </div>
                )}
                {doc.requestedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Requested</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{formatDate(doc.requestedAt)}</span>
                  </div>
                )}
                {doc.reviewedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Reviewed by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.reviewedBy}</span>
                  </div>
                )}
                {doc.reviewedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-600 block">Reviewed</span>
