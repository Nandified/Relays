        ...
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] shadow-[var(--shadow-modal)] backdrop-blur-xl overflow-hidden">
          ...
          <div className="flex items-start justify-between p-6 pb-0">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">{doc.title}</h2>
                <p className="text-xs text-slate-600 dark:text-slate-500 mt-0.5">{meta.description}</p>
              </div>
            </div>
            <button
              onClick={onClose}
              className="flex-shrink-0 rounded-full p-1.5 text-slate-600 dark:text-slate-500 hover:bg-black/5 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-slate-300 transition-colors ml-3"
            >
          ...
          <div className="p-6">
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              {doc.fileSize && (
                <span className="text-xs text-slate-600 dark:text-slate-500">{formatFileSize(doc.fileSize)}</span>
              )}
            </div>
            ...
            {hasFile && (
              <div className="mb-5 rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] overflow-hidden">
                {isImage ? (
                  <div className="relative aspect-[4/3] flex items-center justify-center bg-slate-100 dark:bg-slate-900/50">
                  ...
                  <div className="flex items-center justify-center py-12 bg-slate-100 dark:bg-slate-900/30">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-xs text-slate-600 dark:text-slate-500">PDF Document</span>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-center py-12 bg-slate-100 dark:bg-slate-900/30">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-xs text-slate-600 dark:text-slate-500">Document</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* No file yet */}
            {!hasFile && (
              <div className="mb-5 rounded-2xl border border-dashed border-[var(--border)] bg-black/[0.02] dark:bg-white/[0.02] p-8 text-center">
                <div className="flex flex-col items-center gap-2 text-slate-600 dark:text-slate-500">
                  <svg width="32" height="32" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="text-slate-500 dark:text-slate-500">
            ...
            <div className="space-y-3 text-sm">
              {doc.description && (
                <div>
                  <span className="text-xs text-slate-600 dark:text-slate-500 block mb-0.5">Description</span>
                  <span className="text-slate-700 dark:text-slate-300">{doc.description}</span>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[var(--border)]">
                {doc.uploadedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Uploaded by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.uploadedBy}</span>
                  </div>
                )}
                {doc.uploadedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Uploaded</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{formatDate(doc.uploadedAt)}</span>
                  </div>
                )}
                {doc.requestedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Requested by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.requestedBy}</span>
                  </div>
                )}
                {doc.requestedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Requested</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{formatDate(doc.requestedAt)}</span>
                  </div>
                )}
                {doc.reviewedBy && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Reviewed by</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{doc.reviewedBy}</span>
                  </div>
                )}
                {doc.reviewedAt && (
                  <div>
                    <span className="text-[11px] text-slate-500 dark:text-slate-500 block">Reviewed</span>
                    <span className="text-xs text-slate-700 dark:text-slate-300">{formatDate(doc.reviewedAt)}</span>
                  </div>
                )}
              </div>
            </div>
