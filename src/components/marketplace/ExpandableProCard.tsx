...
export function ExpandableProCard({ pro, expanded, onToggle }: ExpandableProCardProps) {
  ...
  const contentId = `pro-card-${pro.id}`;
          ...
          <div className="relative flex-shrink-0">
            ...
            {pro.introVideoUrl && (
              <button
                type="button"
                onClick={openVideo}
                aria-label={`Play ${pro.name}'s intro video`}
            ...
            <div className="mt-4 border-t border-[var(--border)] pt-4">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg
                      ...
                      width="14"
                      height="14"
                      fill={star <= Math.round(pro.rating) ? "#f59e0b" : "#334155"}
                      viewBox="0 0 20 20"
                    >
                      <path d="M10 1l2.39 4.84L18 6.71l-4 3.9.94 5.49L10 13.63 5.06 16.1 6 10.61l-4-3.9 5.61-.87z" />
                    </svg>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                {pro.badges.slice(0, 2).map((b) => (
                  <Badge key={b.label} variant="accent">
                    {b.label}
                  </Badge>
                ))}
                {pro.verified && <Badge variant="success">✓ Verified</Badge>}
