...
export default function ProRequestDetailPage() {
  ...
  if (!request) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <Link href="/pro/requests" className="mt-4 inline-block text-sm text-[var(--accent)] hover:underline">
          ← Back to Requests
        </Link>
      </main>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <Link href="/pro/requests" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <h1 className="text-xl font-bold text-slate-900">Lead from {request.clientName}</h1>
      ...
      <Card padding="lg" className="mb-6">
        <h2 className="text-sm font-semibold text-slate-900 mb-3">Client Information</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <div className="text-xs text-slate-500">Name</div>
            <div className="text-sm text-slate-900 font-medium">{request.clientName}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Email</div>
            <div className="text-sm text-slate-900">{request.clientEmail}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Service</div>
            <div className="text-sm text-slate-900">{request.category}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Location</div>
            <div className="text-sm text-slate-900">{request.addressOrArea}</div>
          </div>
        </div>
      </Card>
      ...
      {/* Request details */}
      <Card padding="lg" className="mb-6">
        <h2 className="text-sm font-semibold text-slate-900 mb-3">Request Details</h2>
      ...
      <Card padding="lg">
        ...
        <textarea
          className="w-full rounded-xl border border-[var(--border)] bg-white p-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-light)] min-h-[100px] resize-y"
