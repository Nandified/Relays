...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span className="relative flex h-2.5 w-2.5">
      ...
      <span className={`relative inline-flex h-2.5 w-2.5 rounded-full ${bg[status] || "bg-slate-300 dark:bg-slate-500"}`} />
    </span>
  );
}
  ...
  return (
    <Modal open={open} onClose={onClose} title="Webhook Endpoint">
      <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
        ...
        <div>
          <div className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Subscribed Events</div>
          <div className="flex flex-wrap gap-1.5">
            {endpoint.events.map((evt) => (
              <span key={evt} className="px-2 py-0.5 rounded-md text-xs font-mono bg-blue-500/10 text-blue-500 dark:text-blue-400 border border-blue-500/15">{evt}</span>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Recent Deliveries</div>
      ...
      {tab === "api-keys" && (
        <div className="space-y-4">
          ...
          <Card padding="lg" className="border-blue-500/10">
            <div className="flex items-start gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500/10 text-blue-500 dark:text-blue-400 shrink-0">
