...
const POSITION_BORDER = ["border-amber-400/40", "border-slate-400/30", "border-amber-700/30"];
const POSITION_TEXT = ["text-amber-400", "text-slate-400", "text-amber-700"];
        ...
        <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
          {availablePros.map((pro) => (
            <button key={pro.id} onClick={() => { onAdd(pro.id); onClose(); }}
              className="flex w-full items-center gap-3 rounded-xl p-3 text-left border border-transparent hover:bg-white/[0.03] hover:border-[var(--border)] transition-all duration-200">
...
function PartnerCard({ proId, position, onMoveUp, onMoveDown, onRemove, isFirst, isLast }: {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)]/80 p-3.5 group hover:border-[var(--border-hover)] transition-all duration-200">
      <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl border font-bold text-xs ${POSITION_BORDER[posIdx] || "border-[var(--border)]"} ${POSITION_GLOW[posIdx] || ""} bg-white/5`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="text-sm font-medium text-slate-200 truncate">{pro.name}</span>
          {pro.verified && <Badge variant="accent" className="text-[9px] py-0">✓</Badge>}
        </div>
        <div className="text-xs text-slate-500">{pro.companyName}</div>
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <span className="text-[10px] text-slate-600">{pro.reviewCount} reviews</span>
        </div>
      </div>
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0">
        <button onClick={onMoveUp} disabled={isFirst} className="rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Move up">
          <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M5 15l7-7 7 7" /></svg>
        </button>
        <button onClick={onMoveDown} disabled={isLast} className="rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Move down">
...
function ShareLinkPreview({ group }: { group: CuratedGroup }) {
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] p-4 mt-4">
      <div className="text-[10px] text-slate-600 uppercase tracking-wider mb-3">Client preview</div>
        ...
        {PARTNER_CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat} className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-600">{partners.slice(0, 3).map((p) => getProById(p.proId)?.name.split(" ")[0]).filter(Boolean).join(", ")}</span>
            </div>
          );
        })}
      </div>
...
export default function GroupDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-xl font-semibold text-slate-100 mb-1">Group not found</h1>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 pb-20">
      <div className="mb-6">
        <Link href="/pro/groups" className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 transition-colors mb-4">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          {editingName ? (
            <div className="flex items-center gap-2 mb-2">
              <input autoFocus className="bg-transparent text-2xl font-bold text-white outline-none border-b border-blue-500/40 pb-0.5" value={nameValue}
            ...
            <button onClick={() => setEditingName(true)} className="group/name flex items-center gap-2 mb-2">
              ...
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-600 opacity-0 group-hover/name:opacity-100 transition-opacity"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
            </button>
          )}

          {editingDesc ? (
            <div className="mb-4">
              <textarea autoFocus className="w-full bg-transparent text-sm text-slate-400 outline-none border border-[var(--border)] rounded-xl p-2 resize-none" value={descValue} rows={2}
            ...
            <button onClick={() => setEditingDesc(true)} className="group/desc flex items-center gap-2 mb-4">
              <p className="text-sm text-slate-400 text-left">{group.description || "Add a description..."}</p>
              <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-600 opacity-0 group-hover/desc:opacity-100 transition-opacity flex-shrink-0"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
            </button>
          )}
      ...
      <Card padding="md" className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-slate-200">Share Link</h3>
          ...
          return (
            <div key={category} className="rounded-3xl border border-[var(--border)] overflow-hidden">
              ...
              <div className="px-5 pb-5 space-y-2">
                ...
                {partners.length === 0 && (
                  <button onClick={() => setAddingCategory(category)} className="flex w-full items-center justify-center rounded-2xl border border-dashed border-[var(--border)] bg-white/[0.01] p-6 hover:border-[var(--border-hover)] hover:bg-white/[0.02] transition-all">
                    <div className="text-center">
