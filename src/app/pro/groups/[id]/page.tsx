    ...
    <Modal open={open} title={`Add ${category}`} onClose={onClose}>
      <div className="space-y-3">
        ...
        <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
          ...
          {availablePros.length === 0 && <div className="py-6 text-center text-sm text-slate-600 dark:text-slate-400">No {category.toLowerCase()}s available to add.</div>}
        </div>
      </div>
    </Modal>
  );
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)]/80 p-3.5 group hover:border-[var(--border-hover)] transition-all duration-200">
      <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl border font-bold text-xs ${POSITION_BORDER[posIdx] || "border-[var(--border)]"} ${POSITION_GLOW[posIdx] || ""} bg-black/5 dark:bg-white/5`}>
        <span className={POSITION_TEXT[posIdx] || "text-slate-600 dark:text-slate-400"}>{label}</span>
      </div>
      <div className="h-10 w-10 overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] flex-shrink-0">
        <Image src={pro.headshotUrl} alt={pro.name} width={40} height={40} />
      </div>
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400">{pro.reviewCount} reviews</span>
        </div>
      </div>
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0">
        <button onClick={onMoveUp} disabled={isFirst} className="rounded-lg p-1.5 text-slate-600 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Move up">
          <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M5 15l7-7 7 7" /></svg>
        </button>
        <button onClick={onMoveDown} disabled={isLast} className="rounded-lg p-1.5 text-slate-600 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Move down">
          <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" /></svg>
        </button>
        <button onClick={onRemove} className="rounded-lg p-1.5 text-slate-600 dark:text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-colors" title="Remove partner">
...
function ShareLinkPreview({ group }: { group: CuratedGroup }) {
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] p-4 mt-4">
      <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Client preview</div>
        ...
        {PARTNER_CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat} className="flex items-center gap-2">
              <span className="text-[10px] text-slate-600 dark:text-slate-400 w-24 flex-shrink-0 truncate">{cat}</span>
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400">{partners.slice(0, 3).map((p) => getProById(p.proId)?.name.split(" ")[0]).filter(Boolean).join(", ")}</span>
            </div>
          );
        })}
      </div>
...
export default function GroupDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">This group may have been removed.</p>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 pb-20">
      <div className="mb-6">
        <Link href="/pro/groups" className="inline-flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 transition-colors mb-4">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          {editingName ? (
            ...
            <button onClick={() => setEditingName(true)} className="group/name flex items-center gap-2 mb-2">
              ...
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500 dark:text-slate-400 opacity-0 group-hover/name:opacity-100 transition-opacity"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
            </button>
          )}
          ...
          {editingDesc ? (
            ...
            <button onClick={() => setEditingDesc(true)} className="group/desc flex items-center gap-2 mb-4">
              ...
              <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500 dark:text-slate-400 opacity-0 group-hover/desc:opacity-100 transition-opacity flex-shrink-0"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
            </button>
          )}

          <div className="flex items-center gap-3 pt-4 border-t border-[var(--border)]">
            <button onClick={handleToggleDefault} className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-medium border transition-all duration-200 ${group.isDefault ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "border-[var(--border)] text-slate-600 dark:text-slate-400 hover:border-amber-500/20 hover:text-amber-400"}`}>
            ...
            <span className="text-xs text-slate-600 dark:text-slate-400">{group.partners.length} partner{group.partners.length !== 1 ? "s" : ""}</span>
          </div>
        </div>
      </div>
      ...
      <Card padding="md" className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400">Clients who visit this link see your curated partners</p>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between p-5 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-600 dark:text-slate-400">{partners.length}/3 partner{partners.length !== 1 ? "s" : ""}</p>
                  </div>
                </div>
                {partners.length < 3 && (
                ...
                {partners.length === 0 && (
                  <button onClick={() => setAddingCategory(category)} className="flex w-full items-center justify-center rounded-2xl border border-dashed border-[var(--border)] bg-black/[0.01] dark:bg-white/[0.01] p-6 hover:border-[var(--border-hover)] hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-all">
                    <div className="text-center">
                      ...
                      <span className="text-xs text-slate-600 dark:text-slate-400">Add a {category.toLowerCase()}</span>
                    </div>
                  </button>
                )}
              </div>
