...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Simple mock auth check — in production, check for admin role
  // For now, we allow anyone to view admin for demo purposes
  // Uncomment below to enforce:
  // React.useEffect(() => {
  //   if (state.status !== "authed") router.push("/login");
  // }, [state, router]);
  ...
  return (
    <div className="admin admin-bg-mesh flex min-h-screen">
      ...
      <aside
        className={`
          admin-sidebar fixed inset-y-0 left-0 z-50 w-64 border-r border-[var(--border)] bg-[var(--bg-subtle)]/95 backdrop-blur-xl
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="border-t border-[var(--border)] px-3 py-4">
            <Link
              href="/"
              className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-500 hover:bg-white/5 hover:text-slate-300 transition-colors"
            >
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            {state.status === "authed" && (
              <div className="mt-2 px-3">
                <div className="text-xs text-slate-600 truncate">{state.user.email}</div>
              </div>
            )}
          </div>
        </div>
