    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-3 flex items-center justify-between">
        <span className="text-sm text-slate-600 dark:text-slate-400">
          ...
          {totalPages > 1 && <span className="text-slate-500 dark:text-slate-400"> · Page {currentPage} of {totalPages}</span>}
        </span>
      </div>
          ...
          <div className="divide-y divide-[var(--border)] min-w-[800px]">
            {paginated.map((pro) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-xs font-bold border transition-transform group-hover:scale-105 ${
                    pro.claimed
                      ...
                      : "bg-slate-200 dark:bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-500/10"
                  }`}>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-slate-600 dark:text-slate-400 truncate">{pro.companyName}</div>
                  </div>
                </div>
                ...
                <div className="mt-1 lg:mt-0">
                  {pro.rating > 0 ? (
                    <div className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400">
                      ...
                      <span className="text-slate-500 dark:text-slate-400">({pro.reviewCount})</span>
                    </div>
                  ) : (
                ...
                {/* Joined */}
                <div className="mt-1 lg:mt-0 text-[11px] text-slate-600 dark:text-slate-400">
                ...
                <button
                  ...
                  className={`h-7 min-w-[28px] rounded-lg text-xs font-medium transition-all ${
                    page === currentPage
                      ...
                      : "text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-black/[0.04] dark:hover:bg-white/[0.04]"
                  }`}
                >
                  {page}
                </button>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-slate-600 dark:text-slate-400">{selectedPro.companyName}</div>
              </div>
            </div>
...
function InfoField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[11px] font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wide">{label}</div>
