...
export default function RequestDetailPage() {
  ...
  if (!request) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <Link href="/requests" className="mt-4 inline-block text-sm text-[var(--accent)] hover:underline">
          ← Back to Requests
        </Link>
      </main>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      ...
      <h1 className="text-xl font-bold text-slate-900 mb-1">{request.category}</h1>
      <p className="text-sm text-slate-600 mb-6">{request.addressOrArea}</p>
            ...
            return (
              <div key={step} className="flex items-center flex-1 last:flex-initial">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`mt-1 text-[10px] font-medium whitespace-nowrap ${isCurrent ? "text-[var(--accent)]" : isPast ? "text-emerald-600" : "text-slate-400"}`}>
                    {stepLabels[step]}
                  </span>
                </div>
                {i < statusSteps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-1 ${i < currentStepIndex ? "bg-emerald-400" : "bg-slate-200"}`} />
                )}
              </div>
            );
          })}
      ...
      {pro && (
        <Card padding="lg" className="mb-6">
          <h2 className="text-sm font-semibold text-slate-900 mb-3">Assigned Professional</h2>
          <Link href={`/pros/${pro.slug}`} className="flex items-center gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-slate-900 group-hover:text-[var(--accent)] transition-colors">{pro.name}</div>
        ...
        <div className="space-y-0">
          {timeline.map((event, i) => (
            <div key={event.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className={`h-3 w-3 rounded-full ${i === 0 ? "bg-[var(--accent)]" : "bg-slate-300"}`} />
