      ...
      <div className="relative w-full max-w-lg animate-in bg-[var(--bg-card)] border-l border-[var(--border)] shadow-[var(--shadow-elevated)] overflow-y-auto">
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-start justify-between">
            ...
            <button onClick={onClose} className="rounded-full p-1 text-slate-600 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-slate-300">
          ...
          {/* Client Info */}
          <Card>
            <h3 className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Client</h3>
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">{journey.client.email}</p>
              <p className="text-xs text-slate-600 dark:text-slate-400">{journey.client.phone}</p>
            </div>
          </Card>

          {/* Journey Roles */}
          <div>
            <h3 className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Service Team</h3>
            <div className="space-y-2">
              {journey.roles.map((role, i) => (
                <div key={i} className={`rounded-xl border p-3 ${
                  ...
                  {role.assignedProId && (
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Assigned: {role.assignedProId}</p>
                  )}
                  {role.recommendedProIds.length > 0 && (
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">{role.recommendedProIds.length} recommended</p>
                  )}
                </div>
              ))}
            </div>
          ...
          {/* Status */}
          <Card>
            <h3 className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Status</h3>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-600 dark:text-slate-400">Next Step</span>
                <span className="text-xs text-slate-700 dark:text-slate-300">{journey.nextStep}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-600 dark:text-slate-400">Agent</span>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--border)]">
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Property</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider hidden sm:table-cell">Client</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">Agent</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Stage</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Roles</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider hidden xl:table-cell">Created</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-4 py-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 truncate">{journey.address}</p>
                    </div>
                  </td>
