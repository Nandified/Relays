    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <Card padding="lg" className="mb-6 relative overflow-hidden">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <div className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Overall Attainment</div>
            <div className="flex items-baseline gap-2">
              <span className={`text-4xl font-bold ${getAttainmentColor(overallPct)}`}>{overallPct}%</span>
              <span className="text-sm text-slate-600 dark:text-slate-400">{totalActual} / {totalTarget} referrals</span>
            </div>
          </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {/* Trend indicator */}
                      <span className={`flex items-center gap-0.5 text-xs font-medium ${quota.trend === "up" ? "text-emerald-400" : quota.trend === "down" ? "text-red-400" : "text-slate-600 dark:text-slate-400"}`}>
                        {quota.trend === "up" ? "↑" : quota.trend === "down" ? "↓" : "→"}
                        {quota.trendPct}%
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-400">{quota.partnerCompany}</p>
                    ...
                    <div className="mt-1.5 flex items-center gap-4 text-xs text-slate-600 dark:text-slate-400">
                  ...
                  <svg
                    ...
                    className={`text-slate-600 dark:text-slate-400 transition-transform duration-300 flex-shrink-0 mt-1 ${expanded ? "rotate-180" : ""}`}
                  >
                    <path d="M6 9l6 6 6-6" />
                  </svg>
                </div>
                      ...
                      {quota.agents.map((agent) => {
                        ...
                        return (
                          <div key={agent.agentName} className="flex items-center gap-3">
                            ...
                            <div className="text-xs text-slate-600 dark:text-slate-400 min-w-[4rem] text-right">
