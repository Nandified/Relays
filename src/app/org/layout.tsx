      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-4">
            <Link href="/org" className="flex items-center gap-2.5 group">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-slate-100 leading-tight">Luxury Realty</span>
                <span className="text-[10px] text-slate-500 leading-tight">Chicago</span>
              </div>
              <span className="rounded-md bg-blue-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-blue-400 border border-blue-500/15 ml-1">
                ORG
              </span>
            </Link>
            <button
              onClick={() => setSidebarOpen(false)}
              className="rounded-xl p-1.5 text-slate-500 hover:bg-white/5 hover:text-slate-300 lg:hidden"
            >
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
            {navGroups.map((group) => (
              <div key={group.label}>
                <div className="mb-1.5 px-3 text-[10px] font-semibold text-slate-600 uppercase tracking-widest">
                  {group.label}
                </div>
                  ...
                  {group.items.map((item) => (
                    <Link
                      ...
                      className={`
                        ...
                        ${isActive(item.href)
                          ? "bg-blue-500/12 text-blue-300 border border-blue-500/15 shadow-[0_0_10px_rgba(59,130,246,0.1)]"
                          : "text-slate-400 hover:bg-white/[0.04] hover:text-slate-200 border border-transparent"
                        }
                      `}
                    >
                      <span className={`transition-colors ${isActive(item.href) ? "text-blue-400" : "text-slate-500 group-hover/nav:text-slate-400"}`}>
                        {item.icon}
                      </span>
          ...
          <div className="border-t border-[var(--border)] px-3 py-3 space-y-1">
            <Link
              href="/"
              className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-500 hover:bg-white/[0.04] hover:text-slate-300 transition-colors"
            >
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2.5 px-3 py-1.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs text-slate-300 truncate">Victoria Langford</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-[var(--border)] bg-[var(--bg)]/80 backdrop-blur-xl px-4 py-3 lg:hidden">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 hover:bg-white/5"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-100">Luxury Realty</span>
          </div>
        </header>
