...
export default function BookingPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
      <Link href={`/pros/${pro.slug}`} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-6">
      ...
      <h1 className="text-2xl font-bold text-slate-900 mb-2">Request Booking</h1>
      ...
      <Card padding="md" className="mb-6">
        <div className="flex items-center gap-3">
          <div className="h-12 w-12 overflow-hidden rounded-2xl border border-[var(--border)] bg-slate-50">
            <Image src={pro.headshotUrl} alt={pro.name} width={48} height={48} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-slate-900">{pro.name}</div>
            <div className="text-xs text-slate-500">{pro.companyName} • {pro.categories[0]}</div>
          </div>
          <div className="h-9 w-9 overflow-hidden rounded-xl border border-[var(--border)] bg-white">
        ...
        {["date", "time", "confirm"].map((s, i) => (
          <React.Fragment key={s}>
            <div className={`
              ...
              ${step === s ? "bg-[var(--accent)] text-white" : i < ["date", "time", "confirm"].indexOf(step) ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400"}
            `}>
              {i + 1}
            </div>
            <span className={`text-xs ${step === s ? "text-[var(--accent)] font-medium" : "text-slate-400"}`}>
      ...
      {step === "date" && (
        <Card padding="lg">
          <h2 className="text-sm font-semibold text-slate-900 mb-4">Select a Date</h2>
      ...
      {step === "time" && selectedDate && (
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setStep("date")} className="text-xs text-[var(--accent)] hover:underline">
              Change date
            </button>
          </div>
                ...
                className={`
                  ...
                  ${!tw.available
                    ...
                    : selectedWindow === tw.id
                      ? "border-[var(--accent)] bg-[var(--accent-light)] text-[var(--accent)]"
                      : "border-[var(--border)] hover:border-slate-300 text-slate-700"
                  }
                `}
              >
                ...
                {!tw.available && <div className="text-[10px] text-slate-300 mt-0.5">Unavailable</div>}
              </button>
            ))}
          </div>
        </Card>
      ...
      {step === "confirm" && selectedWindowData && (
        <Card padding="lg">
          <h2 className="text-sm font-semibold text-slate-900 mb-4">Confirm Your Booking Request</h2>
          ...
          <div className="rounded-xl bg-slate-50 p-4 mb-4 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-slate-900 font-medium">{isInspector ? "2 hours" : "1 hour"}</span>
            </div>
          </div>
          ...
          <textarea
            className="w-full rounded-xl border border-[var(--border)] bg-white p-3 text-sm placeholder:text-slate-400 outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-light)] min-h-[80px] resize-y mb-4"
