...
export default function BookingPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
      <Link href={`/pros/${pro.slug}`} className="inline-flex items-center gap-1 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 mb-6 transition-colors">
      ...
      <Card padding="md" className="mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-600 dark:text-slate-400">{pro.companyName} • {pro.categories[0]}</div>
          </div>
        ...
        {STEPS.map((s, i) => (
          <React.Fragment key={s}>
            <button onClick={() => goToStep(s)} disabled={i > currentStepIndex} className="flex items-center gap-1.5 flex-shrink-0">
              <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all ${step === s ? "bg-[var(--accent)] text-white shadow-[0_0_12px_rgba(59,130,246,0.35)]" : i < currentStepIndex ? "bg-emerald-500 text-white" : "bg-black/5 dark:bg-white/5 text-slate-500 dark:text-slate-400"}`}>
              ...
              <span className={`text-xs transition-colors hidden sm:inline ${step === s ? "text-blue-400 font-medium" : i < currentStepIndex ? "text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}>{STEP_LABELS[s]}</span>
            </button>
      ...
      {step === "service" && (
        <Card padding="lg">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">Select the type of service</p>
          <div className="space-y-2">
            {SERVICE_TYPES.map((st) => (
              <button key={st.type} onClick={() => { setServiceType(st.type); setStep("date"); }}
                className={`w-full flex items-center gap-4 rounded-xl border p-4 text-left transition-all group ${serviceType === st.type ? "border-[var(--accent)]/40 bg-[var(--accent-light)]" : "border-[var(--border)] hover:border-[var(--border-hover)] hover:bg-black/[0.02] dark:hover:bg-white/[0.02]"}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-600 dark:text-slate-400">{st.description}</div>
                </div>
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{serviceType && getBookingTypeLabel(serviceType)} • {duration >= 120 ? "2 hour hold" : `${duration} min`}</p>
            </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-full bg-emerald-500/60" /><span className="text-[10px] text-slate-600 dark:text-slate-400">Available</span></div>
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-full bg-amber-500/40" /><span className="text-[10px] text-slate-600 dark:text-slate-400">Limited</span></div>
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-full bg-slate-300 dark:bg-slate-600/40" /><span className="text-[10px] text-slate-600 dark:text-slate-400">Unavailable</span></div>
          </div>
          {Object.entries(monthGroups).map(([month, monthDays]) => (
            <div key={month} className="mb-5 last:mb-0">
              <h3 className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">{month}</h3>
                ...
                {monthDays.map((day) => {
                  ...
                  return (
                    <button key={day.date} onClick={() => !isUnavailable && handleDateSelect(day.date)} disabled={isUnavailable}
                      ...
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">{day.dayName}</div>
      ...
      {step === "time" && selectedDate && (
        <Card padding="lg">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">Select 2–3 preferred {duration >= 120 ? "2-hour windows" : "time slots"} — ranked by preference</p>
          <div className="rounded-xl bg-black/[0.02] dark:bg-white/[0.02] border border-[var(--border)] p-3 mb-4">
            ...
            <div className="text-[10px] text-slate-500 dark:text-slate-400">{availableSlots.filter((s) => s.available).length} of {availableSlots.length} slots available</div>
          </div>
              ...
              {availableSlots.map((slot) => {
                ...
                return (
                  <button key={windowId} onClick={() => canSelect && handleWindowToggle(slot)} disabled={!canSelect}
                    className={`relative rounded-xl border p-3 text-center transition-all ${!slot.available ? "border-[var(--border)] bg-black/[0.01] dark:bg-white/[0.01] text-slate-700 cursor-not-allowed opacity-40" : isSelected ? "border-blue-500/30 bg-blue-500/[0.08] text-blue-500 dark:text-blue-400 shadow-[0_0_12px_rgba(59,130,246,0.15)]" : !canSelect ? "border-[var(--border)] text-slate-600 dark:text-slate-400 cursor-not-allowed opacity-60" : "border-[var(--border)] text-slate-700 dark:text-slate-300 hover:border-blue-500/20 hover:bg-blue-500/[0.03]"}`}>
                    ...
                    <div className="text-[10px] text-slate-600 dark:text-slate-400 mt-0.5">to {formatTime(endTime)}</div>
      ...
      {step === "details" && (
        <Card padding="lg">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-5">Help the professional prepare for your appointment</p>
      ...
      {step === "confirm" && (
        <Card padding="lg">
          ...
          <div className="rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border border-[var(--border)] p-4 mb-4 space-y-3">
            <div className="flex justify-between text-sm"><span className="text-slate-600 dark:text-slate-400">Professional</span><span className="text-slate-800 dark:text-slate-200 font-medium">{pro.name}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-600 dark:text-slate-400">Service</span><span className="text-slate-800 dark:text-slate-200 font-medium">{serviceType && getBookingTypeLabel(serviceType)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-600 dark:text-slate-400">Duration</span><span className="text-slate-800 dark:text-slate-200 font-medium">{duration >= 120 ? `${duration / 60} hours` : `${duration} minutes`}</span></div>
            {propertyAddress && <div className="flex justify-between text-sm"><span className="text-slate-600 dark:text-slate-400">Property</span><span className="text-slate-800 dark:text-slate-200 font-medium text-right max-w-[60%]">{propertyAddress}</span></div>}
            {contactName && <div className="flex justify-between text-sm"><span className="text-slate-600 dark:text-slate-400">Contact</span><span className="text-slate-800 dark:text-slate-200 font-medium">{contactName}</span></div>}
          </div>
            ...
            <div className="space-y-2">
              {selectedWindows.map((w, i) => (
                <div key={w.id} className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-black/[0.02] dark:bg-white/[0.02] p-3">
                  ...
                  <div><div className="text-sm text-slate-800 dark:text-slate-200">{formatDate(w.date)}</div><div className="text-xs text-slate-600 dark:text-slate-400">{formatTime(w.startTime)} – {formatTime(w.endTime)}</div></div>
                </div>
              ))}
            </div>
          </div>
