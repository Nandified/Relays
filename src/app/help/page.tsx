          ...
          <div className="relative text-center max-w-2xl mx-auto">
            ...
            <div className="mt-8 max-w-md mx-auto relative">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-card)]/80 backdrop-blur-sm pl-11 pr-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-500 focus:outline-none focus:border-[var(--accent)]/40 focus:ring-1 focus:ring-[var(--accent)]/20 transition-all"
              />
            </div>
          </div>
        </section>
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-4">
              {howItWorks.map((item) => (
                <div key={item.step} className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)]/80 p-6 text-center">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">{item.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {guides.map((guide) => (
              <div key={guide.title} className={`rounded-3xl border ${guide.accent} p-6 h-full`}>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 mb-4">{guide.description}</p>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-medium transition-all cursor-pointer ${
                    activeCategory === cat.id
                      ...
                      : "bg-[var(--bg-card)] border border-[var(--border)] text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 hover:border-[var(--border-hover)]"
                  }`}
                >
                  <span>{cat.icon}</span>
                  {cat.label}
            ...
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)]/80 backdrop-blur-sm p-6 sm:p-8">
              {filteredFAQs.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">No results found. Try a different search or category.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-2xl px-4 py-16 md:py-20">
          ...
          {submitted ? (
            <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/[0.04] p-8 text-center animate-in">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">We&apos;ll get back to you within 24 hours. Check your email for updates.</p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-500 focus:outline-none focus:border-[var(--accent)]/40 focus:ring-1 focus:ring-[var(--accent)]/20 transition-all resize-none"
                  />
                </div>
                <Button
                  className="w-full"
