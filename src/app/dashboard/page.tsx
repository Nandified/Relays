      ...
      <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Link href="/marketplace">
          <Card hover padding="sm" className="text-center py-4">
            <div className="mx-auto h-10 w-10 rounded-xl bg-[var(--accent-light)] flex items-center justify-center mb-2">
              <svg width="18" height="18" fill="none" stroke="#3b82f6" strokeWidth="1.5" viewBox="0 0 24 24">
                <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
              </svg>
            </div>
            <div className="text-xs font-medium text-slate-700">Find Pros</div>
          </Card>
        </Link>
        <Link href="/team">
          <Card hover padding="sm" className="text-center py-4">
            <div className="mx-auto h-10 w-10 rounded-xl bg-emerald-50 flex items-center justify-center mb-2">
              <svg width="18" height="18" fill="none" stroke="#059669" strokeWidth="1.5" viewBox="0 0 24 24">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
                <circle cx="9" cy="7" r="4" />
                <path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" />
              </svg>
            </div>
            <div className="text-xs font-medium text-slate-700">My Team</div>
          </Card>
        </Link>
        <Link href="/requests">
          <Card hover padding="sm" className="text-center py-4">
            <div className="mx-auto h-10 w-10 rounded-xl bg-purple-50 flex items-center justify-center mb-2">
              <svg width="18" height="18" fill="none" stroke="#7c3aed" strokeWidth="1.5" viewBox="0 0 24 24">
                <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2" />
              </svg>
            </div>
            <div className="text-xs font-medium text-slate-700">Requests</div>
          </Card>
        </Link>
        <Card hover padding="sm" className="text-center py-4 cursor-pointer">
          <div className="mx-auto h-10 w-10 rounded-xl bg-amber-50 flex items-center justify-center mb-2">
            <svg width="18" height="18" fill="none" stroke="#d97706" strokeWidth="1.5" viewBox="0 0 24 24">
              <path d="M12 4v16m8-8H4" />
            </svg>
          </div>
          <div className="text-xs font-medium text-slate-700">New Journey</div>
        </Card>
      </div>

      {/* Journey cards (Pending Items OS) */}
...
function JourneyCard({ journey }: { journey: Journey }) {
  return (
    <Card padding="lg" hover>
      ...
      <div className="rounded-xl bg-[var(--accent-light)] border border-blue-100 p-3 mb-4">
        <div className="flex items-start gap-2">
          ...
          <div>
            <div className="text-sm font-medium text-[var(--accent)]">{journey.pendingAction}</div>
            <div className="text-xs text-slate-600 mt-0.5">{journey.nextStep}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-slate-500">
          Owner: <span className="font-medium text-slate-700">{journey.owner}</span>
        </span>
      </div>
        ...
        <div className="flex -space-x-2">
          {journey.teamMembers.slice(0, 5).map((tm) => {
            ...
            return (
              <div
                key={tm.proId}
                className="h-7 w-7 overflow-hidden rounded-full border-2 border-white bg-slate-100"
