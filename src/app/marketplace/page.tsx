  ...
  const IdfprDivider = () => (
    <div className="flex items-center gap-4 py-2">
      ...
      <span className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500 whitespace-nowrap">
        ...
        {idfprTotal.toLocaleString()} IDFPR licensed professionals
      </span>
  ...
  return (
    <main className="mx-auto max-w-6xl overflow-x-hidden px-3 sm:px-4 py-4 sm:py-6">
      ...
      <div className="mb-2 sm:mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs sm:text-sm text-slate-500">
        ...
        {idfprTotal > 0 && (
          <span>
            · <span className="font-medium text-slate-300">{idfprTotal.toLocaleString()}</span> IDFPR licensed
          </span>
        )}
        {filteredPlaces.length > 0 && (
          <span>
            · <span className="font-medium text-slate-300">{filteredPlaces.length}</span> from Google
          </span>
        )}
      ...
      <div className="lg:hidden">
        <div className="flex flex-col gap-1.5">
          ...
          {/* IDFPR divider */}
          {idfprData.length > 0 && <IdfprDivider />}
          ...
          {filteredPlaces.length > 0 && (filteredPros.length > 0 || idfprData.length > 0) && <GoogleDivider />}
